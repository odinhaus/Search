...
namespace Data.Core.Linq
{
    public abstract class DataProvider : QueryProvider, ICreateExecutor, IProvideRepository
    {
        ...
        public virtual Expression GetExecutionPlan(Expression expression)
        {
            ...
            if (provider == null)
            {
                Expression rootQueryable = this.Find(expression, parameters, typeof(IPersistable));
                provider = Expression.Property(rootQueryable, typeof(IPersistable).GetProperty("Provider"));
            }
            // pass in the IPersistableProvider parameter to the builder
            provider = Expression.Parameter(typeof(IPersistableProvider), "provider");
        ...
        class CommandGatherer : ExpressionVisitor
        {
        ...
        public override object Execute(Expression expression)
        {
            ...
            if (lambda != null)
            {
                ...
                var parms = lambda.Parameters
                    ...
                    {
                        (ParameterExpression)this.Find(plan, null, typeof(IPersistableProvider))
                    });

                // make lambda type that extends current lambda to include IPersistableProvider parameter
                var lambdaType = MakeLambdaType(parms, lambda.ReturnType);

                // this lambda will also require input of IPersistableProvider instance as final parameter
        ...
        /// <summary>
        /// Creates a new Func<> type to allow the passing of the IPersistableProvider as the last input parameter
        /// </summary>
        ...
        public override IPersistable CreateQuery(Expression expression)
        {
            Type elementType = expression.Type.ResolveElementType();
            try
            {
                return (IPersistable)Activator.CreateInstance(typeof(DataSet<>).MakeGenericType(elementType), new object[] { this, this.Repository, expression });
            }
        ...
        public override IModelListProvider<TElement> CreateListProvider<TElement>()
        {
            var builder = AppContext.Current.Container.GetInstance<IModelListProviderBuilder>();
            return builder.CreateListProvider<TElement>();
        }
    }
}
